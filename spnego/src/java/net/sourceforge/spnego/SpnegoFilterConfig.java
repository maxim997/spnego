...
import net.sourceforge.spnego.credential.CredentialProvider;
 ...
 * LoginModule options have been set.
 * </p>
 * 
 * <p>
 * To see a working example and instructions on how to use a keytab, take a look
 * at the <a href="http://spnego.sourceforge.net/server_keytab.html" target=
 ...
 * "_blank">installing on tomcat</a> documentation.
 * </p>
 * 
...
final class SpnegoFilterConfig { // NOPMD
    ...
    private static final Logger LOGGER = Logger
	    .getLogger(Constants.LOGGER_NAME);
	...
	String preauthCred = config
		.getInitParameter(Constants.SPNEGO_PREAUTH_CRED);
	
	Properties preauthProps = CredentialProvider
		.getProperties(preauthCred, LOGGER).orElseGet(() -> {
		    Properties props = new Properties();
		    String user = config
			    .getInitParameter(Constants.PREAUTH_USERNAME);
		    if (user != null) {
			props.put(CredentialProvider.USER_NAME_KEY, user);
		    }
		    String pwd = config
			    .getInitParameter(Constants.PREAUTH_PASSWORD);
		    if (pwd != null) {
			props.put(CredentialProvider.PASSWORD_KEY, pwd);
		    }
		    return props;
		});

	String userName = preauthProps
		.getProperty(CredentialProvider.USER_NAME_KEY);
	String password = preauthProps
		.getProperty(CredentialProvider.PASSWORD_KEY);
	...
	if (null != config.getInitParameter(Constants.ALLOW_LOCALHOST)) {
	    this.allowLocalhost = Boolean.parseBoolean(
		    config.getInitParameter(Constants.ALLOW_LOCALHOST));
	}

	// determine if the server supports credential delegation
	if (null != config.getInitParameter(Constants.ALLOW_DELEGATION)) {
	    this.allowDelegation = Boolean.parseBoolean(
		    config.getInitParameter(Constants.ALLOW_DELEGATION));
	}
    }
	...
	final AppConfigurationEntry entry = config
		.getAppConfigurationEntry(moduleName)[0];
	...
	if (opt.containsKey("storeKey")) {
		...
		throw new UnsupportedOperationException(
			"Login Module for server "
				+ "must have storeKey option in login file set to true.");
	    }
	} else {
	    throw new UnsupportedOperationException(
		    "Login Module for server does "
			    + "not have the storeKey option defined in login file.");
	}

	if (opt.containsKey("useKeyTab") && opt.containsKey("principal")
	...
	// we only expect one entry
	final AppConfigurationEntry[] entry = config
		.getAppConfigurationEntry(moduleName);
	...
	if (0 == entry.length) {
	    throw new IllegalArgumentException("The " + side + " module name "
		    + "exists but login module class not defined: "
		    + moduleName);
	}
	...
	if (entry.length > 1) {
	    throw new IllegalArgumentException(
		    "Only one login module class " + "is supported for the "
			    + side + " module: " + entry.length);
	}

	// confirm class name is "com.sun.security.auth.module.Krb5LoginModule"
	if (!entry[0].getLoginModuleName()
		.equals("com.sun.security.auth.module.Krb5LoginModule")) {
     ...
     * 
     * @param basic
     *            true if basic is allowed
     * @param unsecure
     *            true if un-secure basic is allowed
     */
    private void setBasicSupport(final String basic, final String unsecure) {
	if (null == basic) {
	    throw new IllegalArgumentException(
		    SpnegoFilterConfig.MISSING_PROPERTY
			    + Constants.ALLOW_BASIC);
	}

	if (null == unsecure) {
	    throw new IllegalArgumentException(
		    SpnegoFilterConfig.MISSING_PROPERTY
			    + Constants.ALLOW_UNSEC_BASIC);
	}
     ...
     */
    boolean useKeyTab() {
	return (this.canUseKeyTab && this.username.isEmpty()
	...
	buff.append("allowBasic=" + this.allowBasic + "; allowUnsecure="
		+ this.allowUnsecure + "; canUseKeyTab=" + this.canUseKeyTab
